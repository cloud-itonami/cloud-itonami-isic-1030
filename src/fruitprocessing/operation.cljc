(ns fruitprocessing.operation
  ...
  decision to a human operator (plant manager / food-safety officer). The
  ...
  when the Governor is clean -- see `fruitprocessing.phase`.
...
(defn build
  ...
  for testing. opts:
    :advisor -- a `fruitprocessing.advisor/Advisor` (default: mock-advisor)"
  [store & [opts]]
